...
3.5、自旋锁和阻塞锁
3.5.1 什么叫自旋锁？
如果不使用自旋锁，那么久需要阻塞或唤醒一个Java线程，需要操作系统切换CPU状态来完成，这种状态转换需要耗费处理器时间；
...
3.5.3 原理和源码分析
1)、在java1.5版本及以上的并发框架java.util.concurrent的atmoic包下的类基本都是自旋锁的实现；
...
2)、另外，自旋锁适用于临界区比较短小恶毒情况，否则如果临界区很大(线程一旦拿到锁，很久以后才会释放)，那也是不合适的；
