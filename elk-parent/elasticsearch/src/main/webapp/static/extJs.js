...
function progressLoad2(){
    $("<div class=\"datagrid-mask\" style=\"position:absolute;z-index: 9999;\"></div>").css({display:"block",top:30,width:$(window).width()-165,height:$(window).height()}).appendTo('.panel.easyui-fluid');
    $("<div class=\"datagrid-mask-msg\" style=\"position:absolute;z-index: 9999;\"></div>").html("正在加载，请稍候。。。").appendTo(".panel.easyui-fluid").css({display:"block",left:($(document.body).outerWidth(true) - 190) / 2,top:($(window).height() - 45) / 2});
}
function progressClose2(){
