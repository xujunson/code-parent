...
/* 		uploadURL : "/stream/upload",
		...
		onQueueComplete: function(msg) {
			msg=$.parseJSON(msg);
			getAppendFileToList(msg.fileName,msg.attachId,msg.fileType);
...
//		onDestroy: function() {alert('onDestroy')} /** 文件上传出错的响应事件 */
	...
	function getAppendFileToList(fileName,fileid,fileType){
		  var pre = $("#${domId}").val();
		  var now="";
		  if(pre){
		  	now = pre+","+fileid;
		  ...
		  $("#${domId}").val(now);
		  $("#${domIdDisp}").append("<p>"+fileName
				  +pdf
				  +"<a style='margin-left:20px;' href='javascript:void(0)' onClick='downloadAttach(\""+fileid+"\")'>下载</a>"
				  +"<a href='javascript:void(0)' style='margin-left:20px;'  onClick='delAttach(\""+fileid+"\",this,\""+file_upload["attachId"]+"\");'>删除</a><p>"
		 );
	}
</script>
<div class="easyui-layout" data-options="fit:true,border:false">
	...
	<div id="i_select_files" style="width:75px;float:left;"></div>
	<div>
